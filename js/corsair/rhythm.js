...
window._renderRhythmView = function() {
  ...
  if (!items.length) {
    body.innerHTML = '<div class="empty-state" style="text-align:center;padding:120px 24px;color:var(--t3)">' +
      ...
      '<div style="font-size:var(--text-lg);color:var(--t2);margin-bottom:36px;line-height:1.5;max-width:520px;margin-left:auto;margin-right:auto">No overdue commitments, no slipping milestones, no decaying relationships.</div>' +
      '<button onclick="switchView(\'graph\')" style="display:inline-flex;align-items:center;gap:10px;padding:14px 28px;background:var(--gold);color:var(--s1);border:none;border-radius:6px;font-family:\'IBM Plex Sans\',sans-serif;font-size:var(--text-md);font-weight:600;letter-spacing:.02em;cursor:pointer;box-shadow:0 2px 8px rgba(184,105,30,.18);transition:all 180ms ease" onmouseover="this.style.transform=\'translateY(-1px)\';this.style.boxShadow=\'0 6px 18px rgba(184,105,30,.28)\'" onmouseout="this.style.transform=\'\';this.style.boxShadow=\'0 2px 8px rgba(184,105,30,.18)\'">Open Graph<span style="font-size:var(--text-lg);line-height:1">→</span></button>' +
      '</div>';
  ...
  ['critical', 'warn', 'info'].forEach(function(sev) {
    ...
    html += '<div style="font-family:IBM Plex Mono,monospace;font-size:var(--text-2xs);font-weight:700;letter-spacing:.16em;text-transform:uppercase;color:' + cfg.color + ';margin-bottom:10px;display:flex;align-items:center;gap:8px">';
    ...
    arr.forEach(function(it, ix) {
      ...
      html += '<div style="min-width:140px"><div style="font-family:IBM Plex Mono,monospace;font-size:var(--text-2xs);font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:' + cfg.color + '">' + kind + '</div><div style="font-family:IBM Plex Mono,monospace;font-size:var(--text-xs);color:var(--t3);margin-top:2px">' + mtxt + '</div></div>';
      html += '<div style="flex:1;min-width:200px"><div style="font-family:Antonio,sans-serif;font-size:var(--text-lg);color:var(--text);font-weight:700;letter-spacing:-.01em;line-height:1.2">' + subj + '</div>' + (actor ? '<div style="font-size:var(--text-sm);color:var(--t3);margin-top:3px">' + actor + '</div>' : '') + '</div>';
      html += '<button onclick="event.stopPropagation();window._rhythmJumpToGraph(\'' + (graphTarget || '').replace(/\'/g, '&#39;') + '\',\'' + subj.replace(/\'/g, '&#39;') + '\')" title="View in Graph" style="padding:5px 9px;background:var(--amber-bg, rgba(212,130,58,.06));border:1px solid var(--gold);border-radius:2px;color:var(--gold);font-family:IBM Plex Mono,monospace;font-size:var(--text-2xs);font-weight:700;letter-spacing:.12em;text-transform:uppercase;cursor:pointer;flex-shrink:0;transition:background 160ms" onmouseover="this.style.background=\'var(--gold)\';this.style.color=\'var(--s1)\'" onmouseout="this.style.background=\'var(--amber-bg, rgba(212,130,58,.06))\';this.style.color=\'var(--gold)\'">Graph ↗</button>';
      html += '<div style="color:var(--t3);font-size:var(--text-lg);font-weight:300">›</div>';
      html += '</div></div>';
    });
    html += '</div></div>';
  });
