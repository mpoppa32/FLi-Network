...
(function init(){
  ...
  window.Corsair.buildTag    = 'P13.116';
  window.Corsair.buildBlurb  = 'Audit Finding 3.3 batch 1 — _linkifyIntel (every OSINT signal headline/detail renderer routes through it) was regex-replacing URL patterns into anchor tags but never escaping the surrounding text. A hostile OSINT feed (or LLM broker returning prompt-injected output) with <img onerror=...> in a headline executed script the moment innerHTML consumed the result. Now: HTML-entity escape (& < > " \') FIRST, then linkify. URL regex still matches because real URL chars survive escaping; the ampersand-in-query-string case escapes to &amp; which is a valid href. This single fix closes the broadest XSS surface on the platform — every Pulse / Intel Center / signal card render gains the protection in one edit. Per-callsite d.name / opp.name fixes ship as follow-on batches. Critical Findings closed total: 13.';
