...
**STATUS: WIRED, NOT YET EXERCISED.** This is the first workflow file in the repo's history (`git log -- .github` was empty before this commit). Everything below describes what the workflow is *configured* to do; none of it has been observed running. The verified-behavior rewrite lands after the first green run. (Rule 2 — an earlier draft of this section stated all of it in the present indicative as established fact; the reviewer subagent caught it.)

- **`.github/workflows/firebase-deploy.yml`** — configured so a push to `main` touching `functions/**`, `database.rules.json`, or `firebase.json` builds and deploys. Auth via the `FIREBASE_SERVICE_ACCOUNT` GitHub secret (`google-github-actions/auth@v2` → ADC → firebase-tools). `firebase-tools` pinned to `15.17.0`. `--non-interactive` **without** `--force` is deliberate — if the private config fails to restore, the CLI aborts instead of pruning four live production functions.
