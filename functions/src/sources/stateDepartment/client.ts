// State Department RSS client — reuses think_tank's RSS-parsing shape.
//
// Distinct source key ("state_department") for rate-limit / retry scope
// so a State feed outage doesn't burn think_tank's bucket and vice
// versa.
...
export function parseRssFeed(xml: string): StateRssItem[] {
  const items: StateRssItem[] = [];
...
export async function fetchStateFeed(
  rssUrl: string,
  log?: Logger
): Promise<StateRssItem[]> {
  await acquireTokens("state_department", 1);
  const op = async (): Promise<string> => {
    ...
    if (!response.ok) {
      ...
      const err = new Error(
        `State Dept RSS fetch failed: HTTP ${response.status} ${rssUrl} — ${text.slice(0, 160)}`
      );
  ...
  const xml = await withRetry(op, {
    ...
    operationName: "fetch_state_feed",
    log,
  });
