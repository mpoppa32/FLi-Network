// Service-branch news RSS client — reuses think tank RSS pattern
//
// Generic RSS/Atom fetcher. One module reused across all service branches.
...
export async function fetchServiceFeed(rssUrl: string, log?: Logger): Promise<RssItem[]> {
  await acquireTokens("service_news", 1);
  const op = async (): Promise<string> => {
    ...
    if (!response.ok) {
      ...
      const err = new Error(
        `Service news RSS fetch failed: HTTP ${response.status} ${rssUrl} — ${text.slice(0, 160)}`
      );
  ...
  const xml = await withRetry(op, {
    source: "service_news",
    operationName: "fetch_service_feed",
    log,
  });
