...
**State anchor:** commit `26f817b` (P13.403 — dated extraction: one shared `_INTEL_SCHEMA_TAIL`, ISO date + basis alongside the verbatim deadline) — the newest `P13.x` marker on `main`, pushed 2026-09-02. **DEPLOYED BUT NOT ACCEPTED** — no meeting has been reprocessed under the new prompt, so nothing has yet observed what the model emits; see the truth doc's DATED EXTRACTION section for the acceptance procedure. Prior anchor: `b1638c9` (P13.399). Later commits (docs, tests, CI machinery) introduce no new runtime marker, so this anchor holds until the next marked change.
