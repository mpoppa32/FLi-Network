...
**State anchor:** commit `8a74f13` (P13.406 — trust only `meta.ts`; reprocess no longer overwrites `ts`) — the newest `P13.x` marker on `main`, 2026-09-03. **ACCEPTED against live data**: 19/19 contract, datedness 47.4% vs a 38.9% baseline, all resolved dates correct by hand, and `meta.date`/`ts` both survived a reprocess. The P13.403→406 chain (dated extraction · prompt rebalance · stop the normaliser persisting · remove the logging-`ts` fallback) is closed; see the truth doc's DATED EXTRACTION, META.DATE PRESERVATION and THE DATE ANCHOR sections. Prior anchors: `26f817b` (P13.403), `b1638c9` (P13.399). Later commits (docs, tests, CI machinery) introduce no new runtime marker, so this anchor holds until the next marked change.
