...
**Live at:** `https://flisolutions.io/fliintel`. **HOSTED ON NETLIFY — not GitHub Pages.** Corrected 2026-08-30 after an adversarial pass refuted the earlier claim 3/3. Proof, reproducible in one command: `flisolutions.io` resolves to **`75.2.60.5`**, Netlify's load-balancer IP — the value Netlify's own DNS documentation names verbatim — and `www` is a CNAME to `moonlit-truffle-239790.netlify.app`. GitHub Pages' `185.199.108–111.153` block appears **nowhere** in this domain's DNS. Live responses carry `server: Netlify`. The repo-root `CNAME` file only governs the separate GitHub Pages deploy, which 301s *away* to the custom domain; it never had authority over where the domain points. The extensionless `/FLiIntel.html → /fliintel` rewrite is Netlify behaviour GitHub Pages cannot perform — that was the tell, and it was in this doc the whole time.
...
**⚠️ 2026-08-30: both `flisolutions.io` and `/fliintel` returned `503 {"error":"usage_exceeded"}` from Netlify — the site was paused on a hosting usage cap.** `FLiIntel.html` is ~4.4 MB and is re-fetched whole on every uncached load, which makes bandwidth the binding constraint on a low tier. Splitting or compressing that file is a hosting-cost fix, not just a performance one.
