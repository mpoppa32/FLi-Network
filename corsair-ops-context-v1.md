...
0. **MISSION 4, ITEM #4 — MEETING PROCESSING HANGS FOREVER ON A WEDGED SOCKET. Operator-blocking; do this first.** Reproduced twice 2026-08-06: PROCESS → "Analyzing" forever, no error, no console, **zero requests to `anthropicProxy`**, RTDB `.lp` 503 throughout.
...
1. **MISSION 4, ITEM #5 — `commitmentsAutoArchive` nightly job** (ticket: Mike 2026-08-06). Scheduled Firebase function, admin SDK (the only thing that writes headlessly), patterned on `briefSynthesisNightly`. Per-workspace, no hardcoded IDs. Rule: open commitments created >30d ago with **no deadline OR overdue >7d** → `status:'archived'` + `archivedAt` + `archiveNote`. **Never delete.** The daily brief MUST report it when N>0 — one line, `ARCHIVED N STALE (>30d, unscheduled)` — nothing vanishes silently (Rule 11). Staleness selector extracted as a pure exported function, tested Mission-3 style.
   ...
   - **⚠ RULE DISCREPANCY — needs Mike's decision before coding.** The two precedent records were `created 2026-06-24` (43d old) with `deadline 2026-08-05` — i.e. **dated, and only 1 day overdue**. The ticket's rule (no deadline OR overdue **>7d**) would **not** have archived them. The precedent's own note says ">30d old **and overdue**" (any amount). Implementing the ticket literally archives strictly *less* than Mike's manual pass did. Pick one and make the code and the note agree.
