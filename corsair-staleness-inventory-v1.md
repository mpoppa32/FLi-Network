...
## 6b. COUNT CORRECTION — 43 BECAME 45 (41 SWEEPABLE)

The counts in §4 were produced by the ad-hoc audit script that preceded the shipped selector. That script required `Date.parse` to succeed *on top of* the ISO shape match; the shipped `parseIsoDate` never calls `Date.parse` and reads two values the script silently dropped — `"2026-05-25 to 2026-05-27"` (79d overdue) and `"2026-06-08 week"` (65d). **Corrected totals, generated by the shipped selector: 45 selected, 41 to archive, 4 excluded as synthetic.** Nothing had been swept, so the cost was zero — but the lesson is logged: generate the manifest from the production selector, never from a parallel reimplementation.

**Prefix-match vs strict-equal, decided against the corpus** (all 359, 2026-08-12): 70 bare `YYYY-MM-DD` · 7 ISO + delimiter + human text · 0 bare-prefix · 19 date embedded in prose · 263 no date at all. Prefix-match with a required `[T|space]` delimiter shipped. Strict-equal would have missed 6 overdue records; reading embedded dates was also refused, at a measured cost of 8 — because `"Before 2026-09-17"` is a bound, `"During Camp Grayling event (2026-06-06 through ~2026-06-27)"` is a range, and `"~2026-07-27"` is approximate. Both refusals are pinned by fixtures holding the verbatim live values.

## 7. FOUR OF THE 43 ARE SYNTHETIC TEST DATA — EXCLUDE, DO NOT DELETE
...
**Disposition — RULED 2026-08-11: EXCLUDED from the sweep, and NOT deleted.** Archiving it would file fabricated commitments alongside real ones; but deleting it is also wrong, and for a sharper reason than "archive never delete": **removing the meeting would leave the fabricated PEOPLE in the node graph** — a partial cleanup that looks complete. Synthetic-data removal is queued as its own decision, covering the meeting *and* its graph nodes, with its own manifest. The sweep is **41** real overdue action items with these 4 held out.

*(An earlier revision of this section recommended deletion. That was reversed on the graph-residue argument, which is the better one.)*
